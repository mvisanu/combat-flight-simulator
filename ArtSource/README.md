...
The assets were created using the installed Blender MCP add-on's `execute_code` command over its local socket. `Tools/blender_mcp_client.py` is the thin local transport bridge because this agent session did not expose a dedicated Blender MCP tool. No remote asset-generation services are used.
